...
- **Production migrations** are applied via the manual, gated **Migrate (production)** GitHub Action (`.github/workflows/migrate-prod.yml`: `workflow_dispatch` + `DEPLOY` confirm + `production` Environment, using the `DATABASE_URL_PROD` secret). Do **not** run `migrate:deploy:prod` from a laptop against prod for releases; trigger the workflow after taking a Neon snapshot (runbook §B.2). Local `migrate dev` against your own dev DB is still fine for authoring migrations.
