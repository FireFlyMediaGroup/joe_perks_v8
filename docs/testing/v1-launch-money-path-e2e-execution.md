...
## 0. Current sandbox status (2026-06-07)
...
- Connect V2 helper/API smoke — PASS (`v2.core.account` create, status retrieve, `v2.core.account_link` create)
- Connect V2 hosted Express onboarding smoke — PASS (`COMPLETE`, transfer readiness `active`)
- Connect V2 thin-event webhook smoke — PASS for restricted and completed recipient states

This means the **current sandbox validation stack is green** across unit/package, Stripe CLI webhook forwarding, integration-style HTTP/DB flow, buyer-facing browser flow, and Connect V2 recipient onboarding/status/webhook behavior. Connect V2 evidence is recorded in [`2026-06-07-connect-v2-migration-smoke.md`](./2026-06-07-connect-v2-migration-smoke.md).
...
| Connect V2 thin events via `stripe trigger` | Stripe CLI `1.42.1` still does **not** support `stripe trigger v2.core.account[...]`. Use real V2 event ids plus a locally signed thin notification for local smoke tests, or use Dashboard/event-destination delivery in preview/prod. |
...
#### Connect V2 recipient smoke

Checkout still uses platform PaymentIntents plus delayed transfers; Connect V2 validation is about account onboarding/status only. In Stripe test mode:

1. Start a new roaster or org onboarding flow from the portal so `POST /api/stripe/connect` creates a V2 recipient account.
2. Complete the hosted Stripe Express onboarding flow and return to the portal.
3. Confirm the onboarding page reads live Stripe status and mirrors recipient transfer readiness to the existing DB fields.
4. Forward Connect V2 thin events to `localhost:3000/api/webhooks/stripe` and verify the matching `Roaster` / `Org` row updates for `v2.core.account[requirements].updated` and `v2.core.account[configuration.recipient].capability_status_updated`.

2026-06-07 smoke result:

- Incomplete V2 recipient account mapped to `RESTRICTED`, `chargesEnabled = false`, `payoutsEnabled = false`.
- Completed hosted Express onboarding mapped to `COMPLETE`, `chargesEnabled = true`, `payoutsEnabled = true`, with recipient transfer status `active`.
- Real V2 events generated by hosted onboarding included both required event types.
- Local webhook replay with a signed thin notification returned `200`; duplicate replay returned `200`; bad signature returned `400`.
- Plain `curl` cannot fully exercise authenticated portal pages locally because Clerk reports `dev-browser-missing` and Arcjet can deny bot traffic. For full portal click-through, use a real browser Clerk session mapped to `User.externalAuthId`.
