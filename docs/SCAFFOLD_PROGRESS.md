...
| `1.15` | 2026-03-31 | Sprint 3 US-03-02: Admin org approval queue at `apps/admin/app/approvals/orgs/` (list + `[id]` detail, approve → `PENDING_ROASTER_APPROVAL` + `MagicLink` `ROASTER_REVIEW` + `org-roaster-review` email, reject → `org-rejected`). Helpers `_lib/queue-url.ts`, `roaster-org-review-url.ts`. Smoke: `pnpm db:smoke:us-03-02`. Root `.npmrc` `public-hoist-pattern[]=*@sentry*` for Turbopack + `@repo/observability`. |
| `1.16` | 2026-03-31 | Sprint 3 US-03-03: Roaster magic-link org review at `apps/roaster/app/org-requests/[token]/` (`approve-org.ts`, `decline-org.ts`, backup roaster routing). Templates `org-approved`, `org-declined`. Optional `ORG_APP_ORIGIN` on roaster app for org-portal links in email. |
...
| `1.21` | 2026-06-07 | Stripe Connect V2 migration: `@joe-perks/stripe` upgraded to `stripe@^22.2.0`; explicit API-version pin removed; Connect helpers now use V2 recipient accounts/account links/status; roaster/org onboarding pages read live V2 status; `/api/webhooks/stripe` handles V2 thin account events. Sandbox smoke evidence in `docs/testing/2026-06-07-connect-v2-migration-smoke.md` (helper create/status/link, hosted Express onboarding, restricted + complete webhook replay). |
...
| Email scaffold | `Done` | `sendEmail()` in `packages/email/send.ts` (Resend + `EmailLog` dedupe); web contact form uses it. `BaseEmailLayout` shared wrapper, `OrderConfirmationEmail`, `WelcomeEmail`, plus Sprint 2 application templates (`roaster-application-received`, `roaster-approved`, `roaster-rejected`, `org-application-received`). Sprint 3 org pipeline templates: `org-roaster-review`, `org-rejected`, `org-approved`, `org-declined`. |
| Stripe scaffold | `Done` | `@joe-perks/stripe` package complete (client, splits, rate limiting via `limitCheckout` + `limitSlugValidation`, Connect V2 recipient helpers, legacy + V2 status mappers). **`package.json` exports `./splits`** — import `calculateSplits` from `@joe-perks/stripe/splits` in **client** components (main barrel pulls `server-only`). Checkout route creates platform PaymentIntent + DB order with frozen splits; payout remains delayed separate transfers. Webhook handles legacy `account.updated` during migration, V2 thin account events, `payment_intent.succeeded`, `payment_intent.payment_failed`, `charge.refunded`, and dispute events with idempotency. Order-status GET route. Roaster `POST /api/stripe/connect` + onboarding UI; org `POST /api/stripe/connect` + `(authenticated)/onboarding/`. |
