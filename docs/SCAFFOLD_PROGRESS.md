...
| Stripe scaffold | `Done` | `@joe-perks/stripe` package complete (client, splits, rate limiting via `limitCheckout` + `limitSlugValidation`, Connect, status mapper). **`package.json` exports `./splits`** — import `calculateSplits` from `@joe-perks/stripe/splits` in **client** components (main barrel pulls `server-only`). Checkout route creates PaymentIntent + DB order with frozen splits. Webhook handles `account.updated` (roaster + org), `payment_intent.succeeded`, `payment_intent.payment_failed` with idempotency. Order-status GET route. Roaster `POST /api/stripe/connect` + onboarding UI; org `POST /api/stripe/connect` + `(authenticated)/onboarding/`. |
