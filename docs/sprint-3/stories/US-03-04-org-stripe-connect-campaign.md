...
- `apps/org/app/onboarding/page.tsx` exists as **scaffold stub** ("Stripe Connect Express scaffold.")
- `apps/org/app/campaign/page.tsx` exists as **scaffold stub** ("Create and edit campaign scaffold.")
- `apps/org/app/(authenticated)/dashboard/page.tsx` exists
- `apps/org/app/api/webhooks/clerk/route.ts` exists for Clerk user sync
- `apps/roaster/app/(authenticated)/onboarding/` is **fully implemented** (US-02-03) -- use as pattern reference
- `apps/roaster/app/api/stripe/connect/route.ts` is **fully implemented** -- mirror for org
...
- `apps/web/app/api/webhooks/stripe/route.ts` handles `account.updated` for roasters (promote to ACTIVE) -- needs org handling
...
- Server actions for mutations with `requireOrgId()` pattern (create this, mirrors `requireRoasterId()`)
...
- Create `requireOrgId()` helper (mirrors `requireRoasterId()`) for org portal tenant isolation
...
| Create | `apps/org/app/(authenticated)/campaign/_actions/campaign-actions.ts` | Server actions -- create, update, activate campaign |
...
| Create | `apps/org/app/(authenticated)/campaign/_lib/require-org.ts` | `requireOrgId()` tenant auth helper |
...
- [ ] Onboarding page at `/onboarding` shows Stripe Connect status
- [ ] "Start onboarding" button calls `POST /api/stripe/connect` and redirects to Stripe
- [ ] Return from Stripe (`?stripe_return=1`) refreshes status from DB
- [ ] Expired link (`?stripe_refresh=1`) re-initiates onboarding
- [ ] When `stripeOnboarding = COMPLETE` + `chargesEnabled` + `payoutsEnabled`: show success + link to campaign
- [ ] `account.updated` webhook promotes `Org.status` from `ONBOARDING` to `ACTIVE`
- [ ] All data scoped to authenticated org (tenant isolation)

### Campaign

- [ ] Campaign page at `/campaign` is gated: require `Org.status = ACTIVE`
- [ ] Shows approved roaster's products and variants (only active, non-deleted, available)
- [ ] Org admin can select products/variants to include in campaign
- [ ] `CampaignItem` records snapshot `retailPrice` and `wholesalePrice` from `ProductVariant`
- [ ] Campaign name is required
- [ ] `goalCents` is optional (positive int when provided)
- [ ] `orgPct` is set from the org's application `desiredOrgPct` (not user-editable at campaign creation)
- [ ] Campaign starts in `DRAFT` status
- [ ] Activate action: validates at least one `CampaignItem` exists
- [ ] Activate action: validates roaster has at least one `RoasterShippingRate`
- [ ] On activation: `Campaign.status` transitions to `ACTIVE`
- [ ] All campaign mutations scoped to `session.orgId`

---

## Suggested implementation steps

1. Create `requireOrgId()` helper in `_lib/require-org.ts` -- mirrors `requireRoasterId()`.
2. Build the Stripe Connect flow (mirror roaster US-02-03):
   - Create `POST /api/stripe/connect` route (authenticate via Clerk, load User -> Org, create/resume Express account)
   - Build onboarding `page.tsx`, `connect-status.tsx`, `start-onboarding-button.tsx`
   - Handle `?stripe_return=1` and `?stripe_refresh=1`
   - If `apps/org` needs root `.env` values, add `load-root-env.ts`
3. Update webhook handler (`apps/web/app/api/webhooks/stripe/route.ts`):
   - In `account.updated` handler, check if the account belongs to an org (query `Org` by `stripeAccountId`)
   - If org: update `stripeOnboarding`, `chargesEnabled`, `payoutsEnabled`, promote to `ACTIVE` when complete
4. Build campaign creation:
   - Create Zod schema for campaign fields
   - Build server actions: `createCampaign`, `updateCampaign`, `activateCampaign`
   - `createCampaign`: find approved roaster via `RoasterOrgRequest`, load products/variants, create `Campaign` + `CampaignItem` records with price snapshots
   - `activateCampaign`: validate items exist and shipping rates exist, set `status = ACTIVE`
5. Build campaign form UI:
   - Product selector showing roaster's catalog
   - Campaign name + goal input
   - Activate button with guard checks
6. Test: Connect flow, campaign creation, price snapshotting, activation guards, tenant isolation.
...
- The `requireOrgId()` helper should be shared across all org portal features. Consider placing it in `apps/org/app/(authenticated)/_lib/require-org.ts` or a shared location.
