...
- [ ] E2E scenarios MP-01 through EC-24 implemented per [`../testing/money-path-e2e-scenarios.md`](../testing/money-path-e2e-scenarios.md). ⚠️ Code (2026-06-06): **MP-01 + EC-07 + EC-09 + EC-12 implemented** in `tests/e2e/frontend/money-path.spec.ts` (black-box via `GET /api/order-status` + signed webhooks): MP-01 settlement (PENDING→CONFIRMED), EC-09 invalid-signature rejected (400, stays PENDING), EC-07 duplicate-webhook no-op (stays CONFIRMED), EC-12 `charge.refunded`→REFUNDED. MP-01 confirmed green in CI. **Still open:** **MP-02 fulfillment→payout** needs real Stripe **test-mode Connect accounts** in the seed (current seeds use simulated/fake `stripeAccountId`, so a real transfer would fail) — separate infra decision; plus remaining EC-* edge cases.
