...
>
> **Still open** (flagged but not yet coded): order/payout cross-tenant **read** test needs a seeded test DB (A.2); browser `e2e.yml` + money-path scenario coverage, and wiring `web`'s existing unit tests into CI (A.3); `migrate:deploy:prod` not wired into release (A.5); `@repo/observability` context-on-every-log-line not implemented (A.4); incident-comms templates absent (A.4).
