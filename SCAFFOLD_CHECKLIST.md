...
| Stripe (Connect, keys, webhook list) | **Manual** | Test vs live separation for Preview vs Production. Connect V2 code and local sandbox smoke are documented in `docs/testing/2026-06-07-connect-v2-migration-smoke.md`; preview/prod still need real endpoint registration. |
...
| **LB-5** | Full dress-rehearsal deploy to **preview** green: DNS records, env vars, Stripe/Clerk/Inngest webhooks, scripted smoke test. Then repeat on **production**. | **Todo** | Eng lead | Script lives in `docs/runbooks/v1-launch-runbook.md`; Connect V2 local sandbox evidence is in `docs/testing/2026-06-07-connect-v2-migration-smoke.md`; secret-rotation / beta gate checklist lives under Phase A.6b; production data bootstrap process lives in `docs/runbooks/v1-production-bootstrap-checklist.md`. |
...
| **LB-7** | Money-path E2E test committed and running on every PR — scenarios in `docs/testing/money-path-e2e-scenarios.md` | **Partial** | Eng lead | Current sandbox layers are green: `pnpm test`, `pnpm e2e:stripe-listen`, `pnpm e2e:stripe-trigger`, `pnpm e2e:sprint3`, `pnpm test:e2e:frontend`. Still missing full `pnpm test:e2e`, `.github/workflows/e2e.yml`, and the preview dress rehearsal gate. |
