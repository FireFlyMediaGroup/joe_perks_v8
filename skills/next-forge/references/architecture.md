...
### email (Port 3003)
React Email preview server for developing and testing email templates. Templates are React components in the `@joe-perks/email` package.
...
```typescript
import { database } from '@repo-perks/db';
...
bun build --filter @repo-perks/db  # Only the database package
```
