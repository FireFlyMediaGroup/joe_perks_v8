...
Set in `apps/roaster/.env.local` and `apps/web/.env.local` (Joe Perks: no `apps/api` — webhooks live on `apps/web`):
...
Set in `apps/roaster/.env.local` and `apps/web/.env.local`:
...
The schema lives at `packages/db/prisma/schema.prisma`. Edit it, then run `bun run migrate` again after changes.
