...
1. Set the `DATABASE_URL` in `packages/db/.env` pointing to a PostgreSQL database (Neon recommended).
...
| App | Port | Purpose |
|-----|------|---------|
| `web` | 3000 | Marketing site, storefront, checkout, webhooks (`/api/*`) |
| `roaster` | 3001 | Roaster portal (Clerk) |
| `org` | 3002 | Org portal (Clerk) |
| `admin` | 3003 | Platform admin |
| `email` | (varies) | React Email preview server |
| `studio` | 3005 | Prisma Studio for database editing |

Joe Perks removes separate `api`, `storybook`, and Mintlify `docs` apps; use repo-root `docs/` for reference docs.

**Core Packages**: `auth`, `db` (`@joe-perks/db`), `payments`, `email`, `cms`, `design-system`, `analytics`, `observability`, `security`, `storage`, `seo`, `feature-flags`, `internationalization`, `webhooks`, `notifications`, `collaboration`, `ai`, `rate-limit`, `next-config`, `typescript-config`, plus Joe Perks `stripe` and `types`.
...
- `apps/roaster/.env.local` — Roaster portal keys (Clerk, etc.)
- `apps/web/.env.local` — Marketing / storefront / Stripe publishable / web-related keys
- `apps/org/.env.local` — Org portal Clerk keys
- `apps/admin/.env.local` — Admin basic auth (see SCAFFOLD.md)
- `packages/db/.env` — `DATABASE_URL` (required)
...
- `NEXT_PUBLIC_APP_URL` / `NEXT_PUBLIC_WEB_URL` — align with `web` (3000) and `roaster` (3001) per your env schema (`packages/*/keys.ts`).
...
After changing `packages/db/prisma/schema.prisma`:
